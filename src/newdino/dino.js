...
export function Dino({ control = 2, progressive = 1, difficulty = 0, arrowsensitivity = 2 }) {
    ...
    const [obstaclesavoided, setAvoided] = useState(0)
    ...
    function start() {
        ...
        currentDiff > 5 ? setAvoided(1) : setAvoided(2)
    }
    ...
    //useEffct here holds function for posting game data 
    useEffect(() => {
        let interval;
        async function updateData(data) {
            let coins = Math.floor(data.Duration / 8)
            if (coins > 0) { addCoins(uid, coins) }
            await updateStats(uid, 'dino', data)
        }
                    ...
                    for (i = 0; i < obstacle.length; i++) {
                        if (obstacle[i].bottom < 40 && obstacle[i].bottom > 25) {
                            ...
                            if (difference < 10 && stop === false) {
                                ...
                                if (uid) {
                                    updateData({
                                        //every second + 100 points 
                                        Duration: score / 100,
                                        Level: currentDiff + 1, //avoid 0s which will affect calculation of average 
    ...
    return (
        <>
            <div style={{ backgroundColor: 'black', minWidth: '100vw', minHeight: '100vh', display: 'flex' }}></div>

            <Button href='/dino' style={{ top: '5%', left: '5%', position: 'absolute' }}> Back to settings</Button>
            <Button style={{ position: 'absolute', top: `${buttonsLocation[control].top}vh`, right: `${buttonsLocation[control].right}vw`, height: '10vh', width: '10vh', fontSize: '1.5rem' }} onClick={() => { turn('left') }}> {`<`} </Button>
            <Button style={{ position: 'absolute', top: `${buttonsLocation[control].top}vh`, left: `${buttonsLocation[control].left}vw`, height: '10vh', width: '10vh', fontSize: '1.5rem' }} onClick={() => { turn('right') }}> {`>`} </Button>
