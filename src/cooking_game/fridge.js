...
function Instructions(props) {
    return (
        <div>
            <div key='instruction' style={{
                backgroundColor: 'white', width: '50%', height: '50%', top: '25%', left: '15%', position: 'absolute', display: 'flex', flexDirection: 'column', borderRadius: '15px',
                borderColor: '#e7e7e7',
...
export function Fridge({ recipeNo, stepNo, difficulty, next_step }) {
    ...
    const [correctWithNoErrors, setCorrectNo] = useState(0)
    ...
    const handleChange = (e, ui) => {
        ...
        if (newPos.x > 0.7 * (window.innerWidth)) {
            ...
            if (correct_ing_arr.includes(ing_id)) {
                ongoingConsec += 1
                if (consecutive === true) { setCorrectNo(correctWithNoErrors + 1) }
                else if (ongoingConsec > correctWithNoErrors) {
                    setCorrectNo(ongoingConsec)
                    setConsecutive(true)
                }
                ...
                if (task.ing[difficulty].length === 0) {
                    window.sessionStorage.setItem("correctIng", correctWithNoErrors)
                    next_step()
                }
            }
            else {
                ...
                //how to set back as true if they get more coorect?
                setIngredients(prevState => {
