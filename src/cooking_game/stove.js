...
const Stove = ({ recipeNo, stepNo, difficulty, next_step }) => {
    ...
    const [perfect, setPerfectPerc] = useState(0)
    let imgSrc;
    ...
    useEffect(() => {
        setIng(() => {
            ...
            return newState
        }) //generate normal sized ingredients + positions 
        ...
        if (task.adapt.type === 'wait') {
            ...
            for (var c = 0; c < ing_arr.length; c++) {
                let a = 30;
                correct_arr.push(a);
            } //correct time would be half of this, when brightness 100%
        ...
        if (newPos.x < 70 && newPos.x > 30 && newPos.y < 95 && newPos.y > 45) {
            ...
            if (error_margin < 3) {
                ...
                if (error_margin < 1.5) {
                    ...
                    setPerfectPerc(perfect + 1)
                    window.sessionStorage.setItem('perfectTiming', 100 * ((perfect + 1) / ing_arr.length))
                }
            }
            else {
    ...
    return (
        <div className='background'>
            <div className='container'
                ...
                <h1 style={{ textAlign: 'center', margin: '0.1rem', zIndex: '1' }}>{text}</h1>
